...
router.get('/devices', async (req, res) => {
    ...
    try {
        console.log('[GET /internal/devices] Attempting to fetch all devices...');
        const devices = await Device.find().lean(); // 使用 .lean() 获取普通JS对象 (Use .lean() for plain JS objects)
        console.log(`[GET /internal/devices] Successfully fetched ${devices.length} devices. Now fetching statuses...`);
        
        const devicesWithStatus = await Promise.all(devices.map(async (device) => {
            console.log(`[GET /internal/devices] Mapping device: ${device.deviceSN} - Attempting to fetch its status.`);
            console.log(`[GET /internal/devices] Querying DeviceStatus for SN: ${device.deviceSN}`);
            const status = await DeviceStatus.findOne({ deviceSN: device.deviceSN }).lean();
            console.log(`[GET /internal/devices] Status for SN: ${device.deviceSN} - Found: ${status ? 'Yes' : 'No'}`);
            return {
                ...device,
        ...
        console.log('[GET /internal/devices] Successfully processed all device statuses.');
